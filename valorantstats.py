...
def valstats(message):
	try:
		print(message)
		msg = message.content.split('$stats ')
		x = msg[1].split("#")
		embed = None
		URL = "https://tracker.gg/valorant/profile/riot/" + x[0] + "%23" + x[1] + "/overview?season=all"
		...
		if (err != None):
			...
			if (code.text == '404'):
				return discord.Embed(
					title='Error 404: Player not found on Tracker.gg',
					description='Please make sure your Riot ID is linked to TRN. https://thetrackernetwork.com/manage/social'
				)

		colors = [
		discord.Colour.dark_grey(),
		discord.Colour(9127187),
...
#RANK -----------------------------------------------------------------------------------------------------
		...
		cnt = 0
		for i in z:
			embed.add_field(name=giantstats[cnt],
				value=str(i.text),
				inline=True)
			cnt += 1
