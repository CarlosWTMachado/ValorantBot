...
async def agente(ctx):
    ...
    await ctx.message.reply(file=discord.File(chosen['image']),
                            content=f"{chosen['nome']}\n{chosen['gif']}")
...
async def armas(ctx):
    ...
    await ctx.message.channel.send(file=discord.File(arma['image']),
                                   content=arma['nome'])
    await ctx.message.channel.send(file=discord.File(pistola['image']),
                                   content=pistola['nome'])


@client.command()
async def mghm(ctx):
    await ctx.message.channel.send(
        'https://media.discordapp.net/attachments/762328665870434334/915958010470367322/ezgif-4-877403be7b7f.gif'
    )
...
async def timer(ctx, arg1, arg2):
	if arg1 == 's':
		...
		await ctx.message.reply(f"Ja passou {arg2} {arg1}")
	elif arg1 == 'm':
		await asyncio.sleep(int(arg2) * 60)
		await ctx.message.reply(f"Ja passou {arg2} {arg1}")
	elif arg1 == 'h':
		await asyncio.sleep(int(arg2) * 3600)
		await ctx.message.reply(f"Ja passou {arg2} {arg1}")    
...
async def on_message(message):
    ...
    if (message.content == '!work'
            and message.channel.id == int(os.getenv("CHANNEL_COMANDOS"))):
        print("---\nesperar 20 minutos\n---")
    ...
    if (message.content.lower().find('valorant') > -1
            or message.content.lower().find('vava') > -1
            or os.getenv("ROLE_VALORANTES") in message.content):
        await message.reply(
            content=
            f"Opa! Valorant? bora, logando... \n{os.getenv('ROLE_VALORANTES')}\nhttps://tenor.com/view/hasbulla-gif-22466319"
        )
    #ANAO
    if (message.content.lower().find('anao') > -1
            or message.content.lower().find('anão') > -1
            or message.content.lower().find('a nao') > -1
            or message.content.lower().find('ah nao') > -1
            or message.content.lower().find('a não') > -1
            or message.content.lower().find('ah não') > -1):
        await message.reply('https://tenor.com/vILU.gif')

    msgs = message.content.split()
    for i in range(len(msgs) - 4):
        if (msgs[i][0].lower() == 'p'):
            if (msgs[i + 1][0].lower() == "e"):
                if (msgs[i + 2][0].lower() == "n"):
                    if (msgs[i + 3][0].lower() == "i"):
                        if (msgs[i + 4][0].lower() == "s"):
                            msg = "**P**" + msgs[i][1:] + " **E**" + msgs[
                                i + 1][1:] + " **N**" + msgs[
                                    i + 2][1:] + " **I**" + msgs[
                                        i + 3][1:] + " **S**" + msgs[i + 4][1:]
                            await message.reply(f"Hidden PENIS found\n{msg}")
...
async def infoEmb(message):
    ...
    info_embed.add_field(name="$agente",
                         value='escolhe um agente aleatorio',
                         inline=False)
    info_embed.add_field(name="$armas",
                         value='escolhe um arma e um pistola aleatoria',
                         inline=False)
    info_embed.add_field(name="$mghm",
                         value='gif lendario do nosso mestre',
                         inline=False)
    info_embed.add_field(name="$ace (+ @mençao)",
                         value='@mençao com um gif top',
                         inline=False)
